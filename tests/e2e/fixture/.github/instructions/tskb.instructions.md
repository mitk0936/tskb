...
- **__TSKB.ROOT__** (`.`) — The root directory (automatically added by tskb)
    - **api** (`src/api`) — HTTP routes and middleware [1 file]
    - **models** (`src/models`) — Data models and type definitions [3 files]
    - **services** (`src/services`) — Business logic layer [4 files]
    - **utils** (`src/utils`) — Shared utilities [2 files]



## Constraint Docs

- `docs/constraint-service-isolation.tskb.tsx` — Service isolation: services must not import from API layer

Constraint docs define architectural rules that **MUST** be followed when working on related code.

## Essential Docs

- `docs/architecture.tskb.tsx` — Top-level architecture: API → Services → Models with PostgreSQL storage
- `docs/auth.tskb.tsx` — Authentication: JWT-based login, registration, and token refresh

_Plus 2 supplementary docs available via `npx --no -- tskb docs --plain`._

## Externals

- **mailgun** — Outbound transactional email provider (kind: email-service)
- **postgres** — Primary relational database (kind: database)

## Flows

- **auth-login** [essential] — Login request from API route through auth service to database
  api.routes → AuthService → postgres
