...
var f = (r, t, e) => Jt(r, typeof t != "symbol" ? t + "" : t, e);
import {
  h as Zt,
  t as Qt,
  s as k,
...
function U(r, ...t) {
  return r === "folder" ? `./chunks/folder-${ae(t[0])}.json` : `./chunks/${r}.json`;
}
function ae(r) {
...
class le {
  constructor() {
    f(this, "cache", new ie(50));
    f(this, "inflight", new Map());
  }
  async load(t, ...e) {
    const s = t === "folder" ? U("folder", e[0]) : U(t),
      n = this.cache.get(s);
    if (n) return n;
...
const ct = (r) => r.detail._ghost === "true",
  ...
  de = (r) => parseInt(r.detail._childCount ?? "0", 10),
  w = {
    folder: { w: 190, h: 52 },
    module: { w: 165, h: 48 },
    export: { w: 145, h: 40 },
...
function pe(r, t) {
  ...
  const e = Mt(r.meta.topFolders, r, t),
...
function Mt(r, t, e) {
  return r.map((s) => {
    ...
    if (!n) return { ...s };
    const o = [];
    (a = n.subfolders) != null && a.length && o.push(...Mt(n.subfolders, t, e));
    for (const i of n.modules)
      if (e(i)) o.push({ ...i, treeChildren: ue(i.id, n.exports) });
      else if (dt(i)) {
...
function fe(r, t) {
  var h;
  const e = [];
  let s = J + R * 2 + w.folder.h;
  const n = pe(r, t);
  if (n) {
    const d = Math.max(...Object.values(w).map((x) => x.h)),
      u = Math.max(...Object.values(w).map((x) => x.w)) + mt,
      p = Zt(n, (x) => x.treeChildren);
    Qt().nodeSize([d + he, u])(p);
    let E = 1 / 0;
    (p.each((x) => {
      x.x < E && (E = x.x);
    }),
      p.each((x) => {
        x.depth !== 0 && e.push({ ...x.data, x: R + x.y - u, y: J + R + x.x - E });
      }),
      (s = Math.max(...e.map((x) => x.y + w[x.type].h)) + R));
  }
  const o = s + yt,
    a = [],
    i = o,
  ...
  (h = r.meta) != null &&
    ...
    r.meta.externals.forEach((d, u) => {
      ...
      l.push({ ...d, x: R + u * (p + mt), y: i + J + R });
    });
  const c = l.length ? J + R * 2 + w.external.h : 0;
  return {
    structureNodes: e,
    docsNodes: a,
    otherNodes: l,
...
function me(r, t, e, s, n, o, a) {
  y && ((Nt = n), (Ot = o), be(r, t, e, s, a), (y.style.display = "block"), Bt(), we());
}
function ge(r, t) {
  ((z = r), (O = t), y && y.style.display !== "none" && Bt());
}
function xe() {
  y &&
    (cancelAnimationFrame(it),
    (y.style.opacity = "0"),
    (y.style.transform = "translateY(6px)"),
    setTimeout(() => {
      y && y.style.opacity === "0" && (y.style.display = "none");
    }, 120));
}
function be(r, t, e, s, n) {
  if (!y) return;
  const o = n ? Object.entries(n).filter(([d, u]) => d !== "desc" && u !== null && u !== "") : [],
    ...
    h = a
      ? '<div style="display:grid;grid-template-columns:auto 1fr;column-gap:8px;row-gap:2px;font-size:11px;line-height:1.4;max-width:220px;border-top:1px solid #e2e8f0;padding-top:6px;">' +
        ...
        "</div>"
      : "";
  y.innerHTML = `<div style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:16px;padding:9px 14px;box-shadow:0 2px 12px rgba(0,0,0,0.06);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">${i}${l}${c}${h}</div>`;
}
function we() {
  (cancelAnimationFrame(it),
    (it = requestAnimationFrame(() => {
      y && ((y.style.opacity = "1"), (y.style.transform = "translateY(0)"));
    })));
}
function Bt() {
  if (!y || !O) return;
  const r = Nt * z.k + z.x + O.left,
    t = Ot * z.k + z.y + O.top;
  ke(r, t);
}
function ke(r, t) {
  ...
  const e = 14,
    ...
    o = r - s - e,
    a = t - n / 2;
  ((y.style.left = `${Math.max(8, Math.min(o, window.innerWidth - s - 8))}px`),
    (y.style.top = `${Math.max(8, Math.min(a, window.innerHeight - n - 8))}px`));
}
function P(r) {
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const Ce = [["path", { d: "M5 12h14" }]],
  xt = ve,
  bt = Ce;
function Le(r, t, e = 12, s = "currentColor") {
  const n = e / 24,
    o = -e / 2,
    a = r
      .append("g")
...
function wt(r, t, e = 12, s = "currentColor") {
  (r.select(".lucide-icon").remove(), Le(r, t, e, s));
}
const C = {
  folder: "#0057a1",
  module: "#ca8a04",
    ...
    (t
      .on("mouseover", function (n, o) {
        ...
        if (!n.currentTarget.contains(n.relatedTarget)) {
          ...
          me(l, o.path, o.description, C[o.type], o.x, o.y + i / 2, c);
        }
      })
      .on("mouseout", function (n) {
        n.currentTarget.contains(n.relatedTarget) || xe();
      }),
      t
        .append("rect")
        .attr("class", "node-bg")
    ...
    t.each(function (s) {
      ...
      if (
        ...
        (n
          ...
          .text(kt(((ft = s.path) == null ? void 0 : ft.split("/").pop()) || s.label, L)),
      ...
      const p =
        ...
        g = s.type === "folder" || s.type === "file" || s.type === "module",
        E = Math.floor((o - h - 14) / 5.5);
      let $;
      if (g && s.path) {
        const L = s.path.split("/").pop() ?? s.label;
        s.parentId ? (s.type === "folder" ? ($ = `/${L}`) : ($ = L)) : ($ = `/${s.path}`);
      } else s.type === "export" ? ($ = jt(s.label, s.detail.morphology)) : ($ = s.label);
      n.select(".node-label")
        .attr("x", h)
        .attr("y", d / 2)
        .text(kt($, E));
      ...
      if (pt.length > 0) {
        const L = `☰ Docs (${pt.length})`,
          S = L.length * 4.6 + 8;
        (G.style("display", "block").attr("transform", `translate(${st},${x})`),
          G.select("rect").attr("width", S).attr("fill", Gt).attr("stroke", "none"),
          G.select("text")
            .attr("x", S / 2)
            .attr("y", 6.5)
            .attr("fill", Wt)
      ...
      if (ut.length > 0) {
        const L = `→ Flows (${ut.length})`,
          S = L.length * 4.6 + 8;
        (W.style("display", "block").attr("transform", `translate(${st},${x})`),
          W.select("rect").attr("width", S).attr("fill", qt).attr("stroke", "none"),
          W.select("text")
            .attr("x", S / 2)
            .attr("y", 6.5)
            .attr("fill", Vt)
      ...
      const V = n.select(".node-expand-btn");
      if ((V.style("display", l ? "block" : "none"), l)) {
        (V.attr("transform", `translate(${o},${a / 2})`), V.select("circle").attr("stroke", i));
        const L = e.isExpanded(s) ? bt : xt;
        wt(V, L, 10, i);
      }
    });
  }
...
function Se(r, t, e, s, n, o, a, i, l) {
  return new Ee(r, t, e, s, n, o, a, i, l);
}
function Te(r) {
  const t = new Map();
...
function Ae(r, t) {
  ...
  (e.exit().remove(),
    e
      ...
      .attr("stroke-width", 1.5)
      .merge(e)
      .attr("stroke", (n) => C[n.targetType] + (n.ghost ? "59" : "66"))
      ...
      .attrTween("d", function (n) {
        ...
        const h = K(a, n.sourceX),
          d = K(i, n.sourceY),
          u = K(l, n.targetX),
          p = K(c, n.targetY);
        return (g) => Be(h(g), d(g), u(g), p(g));
      }));
}
function _e(r, t, e) {
  const s = e,
  ...
  if (r.type === "imports" || r.type === "imports-type") {
    const s = r.type === "imports-type" ? "imports type from" : "imports from",
      o =
        ...
        Me(r.source.path, r.target.path) ??
        r.target.path ??
        r.target.label;
...
function Ne(r, t) {
  ...
  const n = s.merge(e);
  (n.each(function (o) {
    const { sx: a, sy: i, tx: l, ty: c, fromColor: h } = Ft(o.source, o.target, o.type),
  ...
  (o.append("circle").attr("r", 9).attr("fill", "#ffffff").attr("stroke", "none"),
    ...
    s.merge(e).each(function (i) {
      const {
          ...
          fromColor: u,
          toColor: p,
        } = Ft(i.source, i.target, i.type),
        g = k(this);
      (g.select(".relation-end-src").attr("transform", `translate(${l},${c})`),
        g
          .select(".relation-end-src .arrow-icon")
          .attr("stroke", u)
          .attr("d", "M -3,3 L 3,-3 M 0.5,-3 L 3,-3 L 3,-0.5"),
        g.select(".relation-end-tgt").attr("transform", `translate(${h},${d})`),
        g
          ...
          if (
            ...
            return new Promise((p, g) => {
              (u.addEventListener("load", p),
                u.addEventListener("error", () => g(new Error(`Unable to preload CSS for ${c}`))));
            });
        })
      );
    }
...
function Ke(r) {
  ...
  ((t.textContent = tn), document.head.appendChild(t));
}
function Ue(r, t) {
  ((Y = r), (j = t), B && Yt());
}
async function Je(r, t, e, s, n, o, a) {
  if (B === r) {
    ot();
    return;
  }
...
`)));
  ...
  if (!m) return;
  ((m.innerHTML = `<div class="code-tooltip-header" style="padding:5px 6px 5px 14px;background:#f8fafc;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;gap:4px;"><span style="min-width:0;font-size:12px;color:#94a3b8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;letter-spacing:0.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${en(e)}</span><div class="header-actions" data-slot="path"></div><div style="flex:1"></div><div class="header-actions" data-slot="end"></div></div><pre style="margin:0;padding:10px 16px 12px;overflow:auto;white-space:pre;font-family:'Cascadia Code','JetBrains Mono','Fira Code',monospace;font-size:11px;line-height:1.7;color:#1e293b;flex:1;min-height:0;user-select:text;"><code>${h}</code></pre>`),
    m
      .querySelector('.header-actions[data-slot="path"]')
      .appendChild(et("copy", { title: `Copy ${e}`, onClick: () => Pt(e) })),
    m
    ...
    Yt(),
    requestAnimationFrame(() => {
      m && ((m.style.opacity = "1"), (m.style.transform = "translateY(0) scale(1)"));
    }),
    H && document.removeEventListener("mousedown", H, !0),
    (H = (p) => {
      m && !m.contains(p.target) && ot();
    }),
    setTimeout(() => {
      H && document.addEventListener("mousedown", H, !0);
    }, 0));
}
function ot() {
  m &&
    ...
    setTimeout(() => {
      m && !B && (m.style.display = "none");
    }, 150));
}
function Yt() {
  if (!j) return;
  const r = Dt * Y.k + Y.x + j.left,
    t = zt * Y.k + Y.y + j.top;
  Ze(r, t);
}
function Ze(r, t) {
  ...
  const e = 14,
    s = m.offsetWidth || 620,
    n = m.offsetHeight || 220,
    o = window.innerWidth,
    a = window.innerHeight,
    i = r + e + s > o - 8 ? r - s - e : r + e,
    l = t + e + n > a - 8 ? t - n - e : t + e;
  ((m.style.left = `${Math.max(8, i)}px`), (m.style.top = `${Math.max(8, l)}px`));
}
const Qe = (async () => {
    const [
      { default: r },
    ...
    : r
        ...
        .map((n, o) => {
          ...
          const a = n.priority
            ...
            l = ((E = n.description) == null ? void 0 : E.trim()) || i,
            c = `<span class="accordion-headline">${A(l)}</span>`,
            h = `<a class="tskb-ref accordion-path-ref" data-node-id="${A(n.id)}" data-node-display="${A(i)}" data-no-rewrite title="Open ${A(i)} in graph">${A(i)}</a>`,
            d = `<button type="button" class="accordion-path-copy" data-copy-path="${A(i)}" title="Copy path" aria-label="Copy path"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button>`,
            u = `<div class="accordion-path">${h}${d}</div>`;
          let p;
          if (t === "flows") p = `<div class="accordion-body">${u}${pn(n, e)}</div>`;
          else {
            const $ = n.detail.html;
            p = $
              ? `<div class="accordion-body">${u}${$}</div>`
              : `<div class="accordion-body">${u}</div>`;
          }
  ...
  (e.addEventListener("click", n),
    ...
    e.querySelectorAll("a.tskb-ref").forEach((o) => {
      ...
      const i = o.hasAttribute("data-no-rewrite"),
        l = t.getNode(a);
      (i || (o.textContent = fn(l, o, a)), (o.title = (l == null ? void 0 : l.path) ?? a));
      let c = !1;
...
function un(r) {
  ...
  const s = (n) => {
    var i;
    const o = (i = n.target) == null ? void 0 : i.closest("button[data-copy-path]");
    if (!o) return;
    (n.preventDefault(), n.stopPropagation());
    const a = o.getAttribute("data-copy-path");
    a && Pt(a, a);
  };
  (t.addEventListener("click", s), (t[Tt] = s));
}
function fn(r, t, e) {
  return r
    ? r.type === "export"
      ? jt(r.label, r.detail.morphology)
      : r.type === "folder"
        ? (r.path ?? r.label) + "/"
        : (r.path ?? r.label)
...
function _t(r, t, e) {
  const s = t.getAttribute("data-node-display") ?? e,
    ...
    a = o ? (C[o] ?? "#64748b") : "#64748b";
  sn(n, r == null ? void 0 : r.path, (r == null ? void 0 : r.description) ?? "", a, t);
}
const Z = (r) =>
    r.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"),
  X = class X {
    ...
    renderBody(t) {
      ...
      ((t.innerHTML = hn(e, this.kind, { getNode: this.deps.getNode })), At(t, this.deps), un(t));
    }
  };
f(X, "prefix", "refs");
let nt = X;
...
function xn(r, t) {
  ...
  const n = s.map(([a, i], l) => [a + It(t, l * 2) * Ht, i + It(t, l * 2 + 1) * Ht]);
  return ee()
    .x((a) => a[0])
    .y((a) => a[1])
    .curve(ne)(n);
  ...
  for (const [o, a] of s) {
    ...
    const c = Math.min(...a.map((p) => p.y)),
      ...
      d = Math.max(
        ...a.map((p) => {
          var g;
          return p.x + (((g = w[p.type]) == null ? void 0 : g.w) ?? 160);
        })
      ),
      u = (h + d) / 2;
...
class I {
  ...
  setupCanvas() {
    ...
    const s = se()
      ...
      .on("zoom", (n) => {
        ...
        (Ue({ k: o, x: a, y: i }, l), ge({ k: o, x: a, y: i }, l), Xt(this.boundaryLayer, o));
      });
  ...
  setupTooltips() {
    (ye(this.svgEl), Ke(this.svgEl), nn());
  }
  setupRenderer() {
    ...
    this.renderer = Se(
      ...
      (e) => {
        const s = e.detail.code,
          ...
          a = e.detail.fileType;
        Je(e.id, s, e.path ?? e.id, e.x + o / 2, e.y, n, a);
      },
  ...
  render() {
    ...
    if ((console.log("[render] start"), this.layoutDirty || !this.cachedLayout)) {
      ...
      ((this.cachedLayout = fe(this.store, (g) => this.expanded.has(I.expandKey(g)))),
        (this.layoutDirty = !1),
    ...
    ((document.getElementById("stats").textContent = c
      ...
      .join(" · ")),
      (e = performance.now()),
      _e(this.laneBgLayer, this.cachedLayout, n),
      console.log(`[render] renderLaneBands (${(performance.now() - e).toFixed(1)}ms)`),
      (e = performance.now()),
      kn(this.boundaryLayer, s, this.store.meta.parentOf ?? {}, this.zoomK),
    ...
    if (t.type === "folder")
      if (this.expanded.has(e))
        ...
        if (!this.store.folderChunks.has(t.id)) {
          const s = De(this.nodeLayer.node(), t.x + 190, t.y + 38);
          try {
  ...
  scrollToNode(t, e = !1) {
    ...
    if (e) {
      const h = Math.min(Math.max(this.zoomK, $n), 1.2),
        d = re
