...
function howToRun(model: SkillModel): string[] {
  ...
  return [
    ...
    "completion and returns its verdict — with what it returned as `value` (an action's result",
    "comes back this way). `start_om` starts a long-lived one and returns a run id to poll with",
    "`get_run`, read with `tail_run`, and end with `cancel_run`; pass `waitUntilUp: true` to get",
    "the id back only once the workflow's body has returned — for a dev stack, once it is up.",
    "",
