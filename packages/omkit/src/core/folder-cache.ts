...
export const cacheDir = join(process.cwd(), "node_modules", ".cache", "omkit");
 ...
 * before hashing.
 */
export const fingerprint = async (paths: string[]): Promise<string> => {
...
const entryPath = (paths: string[]): string => {
  const key = createHash("sha1").update(resolvePaths(paths).sort().join("\0")).digest("hex");
 ...
 */
export const read = async (paths: string[]): Promise<string | undefined> => {
  try {
    const raw = await readFile(entryPath(paths), "utf8");
 ...
 */
export const write = async (paths: string[], fp: string): Promise<void> => {
  await mkdir(cacheDir, { recursive: true });
  const target = entryPath(paths);
