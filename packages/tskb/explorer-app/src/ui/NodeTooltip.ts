...
/** Show tooltip anchored to a DOM element — used by DocPanel ref links. */
export function showNodeTooltipAtScreen(
  label: string,
  path: string | undefined,
  description: string,
  color: string,
...
function positionFromElement(anchorEl: HTMLElement): void {
  if (!el) return;
  const r = anchorEl.getBoundingClientRect();
  placeAt(r.left, r.top + r.height / 2);
}

function placeAt(screenX: number, screenY: number): void {
  if (!el) return;
  const GAP = 14;
